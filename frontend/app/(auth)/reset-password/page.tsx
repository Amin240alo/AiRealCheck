...
function ResetPasswordContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!token) { setError('Reset-Link fehlt.'); return; }
    ...
    try {
      ...
      setError(resolveAuthError(err as Parameters<typeof resolveAuthError>[0], 'reset'));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="w-full max-w-sm">
      <div className="rounded-[var(--radius-xl)] bg-[var(--color-surface)] border border-[var(--color-border)] shadow-[var(--shadow-lg)] p-8">
        <div className="mb-6">
          <div className="text-[11px] font-semibold uppercase tracking-widest text-[var(--color-primary)] mb-1">Neues Passwort</div>
          <h2 className="text-[22px] font-bold text-[var(--color-text)]">Passwort festlegen</h2>
          <p className="text-[13px] text-[var(--color-muted)] mt-1">Wähle ein neues Passwort mit mindestens 8 Zeichen.</p>
        </div>
        {done ? (
          <div className="text-center">
            <CheckCircle className="mx-auto mb-3 text-[var(--color-success)]" size={36} />
            <p className="text-[14px] text-[var(--color-text)] mb-4">Passwort aktualisiert.</p>
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
              {[
                { id: 'password', value: password, onChange: setPassword, show: showPw, onToggle: () => setShowPw(!showPw), label: 'Neues Passwort', ac: 'new-password' },
                { id: 'confirm', value: confirm, onChange: setConfirm, show: showPw, onToggle: () => setShowPw(!showPw), label: 'Passwort bestätigen', ac: 'new-password' },
              ].map(f => (
                <div key={f.id} className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    <input id={f.id} type={f.show ? 'text' : 'password'} value={f.value} onChange={e => f.onChange(e.target.value)}
                      placeholder="Mindestens 8 Zeichen" autoComplete={f.ac} required
        ...
        <div className="mt-5 text-center">
          <Link href="/login" className="text-[12px] text-[var(--color-muted)] hover:text-[var(--color-text)] transition-colors">← Zum Login</Link>
        </div>
      </div>
    </motion.div>
  );
