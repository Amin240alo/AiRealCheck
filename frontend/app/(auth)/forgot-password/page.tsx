...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!email) { setError(t('auth.forgot.missingEmail')); return; }
    setError(''); setLoading(true);
    try {
      await forgot(email.trim().toLowerCase());
      setSent(true);
    } catch (err: unknown) {
      setError(t(resolveAuthErrorKey(err as Parameters<typeof resolveAuthErrorKey>[0], 'forgot')));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="w-full max-w-sm">
      <div className="rounded-[var(--radius-xl)] bg-[var(--color-surface)] border border-[var(--color-border)] shadow-[var(--shadow-lg)] p-8">
        <div className="mb-6">
          <div className="text-[11px] font-semibold uppercase tracking-widest text-[var(--color-primary)] mb-1">{t('auth.forgot.badge')}</div>
          <h2 className="text-[22px] font-bold text-[var(--color-text)]">{t('auth.forgot.title')}</h2>
          <p className="text-[13px] text-[var(--color-muted)] mt-1">{t('auth.forgot.subtitle')}</p>
        </div>
        {sent ? (
          <div className="text-center">
            <CheckCircle className="mx-auto mb-3 text-[var(--color-success)]" size={36} />
            <p className="text-[14px] text-[var(--color-text)] mb-2">{t('auth.forgot.sentTitle')}</p>
            <p className="text-[12px] text-[var(--color-muted)] mb-6">{t('auth.forgot.sentText')}</p>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
              <div className="flex flex-col gap-1.5">
                <label htmlFor="email" className="text-[11px] font-semibold uppercase tracking-wider text-[var(--color-muted)]">{t('auth.forgot.emailLabel')}</label>
        ...
        <div className="mt-5 text-center">
          <Link href="/login" className="text-[12px] text-[var(--color-muted)] hover:text-[var(--color-text)] transition-colors">{t('auth.forgot.backToLogin')}</Link>
        </div>
      </div>
    </motion.div>
  );
