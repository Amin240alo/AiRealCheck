...
export function Header({ onMenuClick }: HeaderProps) {
  ...
  const menuItems = [
    { label: 'Profil',        icon: <User size={14} aria-hidden />,        href: '/profile' },
    { label: 'Einstellungen', icon: <Settings size={14} aria-hidden />,    href: '/settings' },
    ...(isAdmin ? [{ label: 'Admin-Panel', icon: <ShieldCheck size={14} aria-hidden />, href: '/admin' }] : []),
  ];
        ...
        {isLoggedIn ? (
          <div className="flex items-center gap-2" ref={dropRef}>
            ...
            <div
              ...
              aria-label={`${creditsAvail} Credits verfügbar`}
            >
              ...
              <span className="text-[var(--color-muted-2)]" aria-hidden="true">Credits</span>
            </div>
            ...
            <div className="relative">
              <button
                ...
                aria-label={`Nutzerprofil: ${user?.display_name || user?.email || 'Profil'}`}
              >
