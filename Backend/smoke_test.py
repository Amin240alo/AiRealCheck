...
r = client.post(
    "/auth/register",
    json={"email": "test@demo.de", "password": "Pass1234", "display_name": "Test User"},
)
...
try:
    ...
    if user:
        ...
        session.add(user)
        session.commit()
        target = int(os.getenv("AIREALCHECK_FREE_CREDITS", "100"))
        current = get_credit_balance(user.id, db=session)
        delta = target - current
        if delta != 0:
            add_ledger_entry(user.id, delta, reason="test_adjust", ref_type="test", ref_id="init", db=session)
            session.commit()
finally:
    session.close()
...
try:
    ...
    if user:
        ...
        delta = target - current
        if delta != 0:
            add_ledger_entry(user.id, delta, reason="test_adjust", ref_type="test", ref_id="reduce", db=session)
            session.commit()
finally:
    session.close()
...
guest_buf = io.BytesIO()
guest_img.save(guest_buf, format="JPEG")
guest_buf.seek(0)
guest_call = client.post(
    "/analyze/guest",
    data={"file": (guest_buf, "guest.jpg")},
